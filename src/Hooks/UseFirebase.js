...
useEffect(()=>{
  ...
  const url =`https://stormy-everglades-14844.herokuapp.com/user/${user.email}`
  axios.get(url)
  .then((res)=>{
...
const saveUser = (email,name,method,location,navigate)=>{
  console.log("save user called !!!");
  const user = {email,name};
  const url=`https://stormy-everglades-14844.herokuapp.com/addUser`
  if(method==="POST")
  {
    axios.post(url,{user})
    .then((res)=>{
...
setIsLoading(true);
    ...
    .then((userCredential) => {
      ...
      setNewName(name);
      saveUser(email,name,'POST',location,navigate);
...
const LogInUser = (email,password,location,navigate)=>{
  ...
  .then((userCredential) => {
    const url =`https://stormy-everglades-14844.herokuapp.com/user/${email}`
    axios.get(url)
    .then((res)=>{
      console.log("admin checking data",res.data.admin);
      if(res.data.admin){
